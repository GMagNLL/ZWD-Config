         ...
         or w == '7' or w == '8' or w == '9' or w == '0' or w == 'm' or w == 'M'\
          or w == 'monitor' or w == 'Monitor' or w == 'MONITOR':
           if w == 'm' or w == 'M' or w == 'monitor' or w == 'Monitor' or w == 'MONITOR':
              ...
              os.system('''OS=`uname -s` && REV=`uname -r` && MACH=`uname -m` &&
              if [ "${OS}" = "SunOS" ]; then
                  OS=Solaris
                  ARCH=`uname -p`
           ...
           if w == '1':
              ...
              if os.path.isfile('/etc/setup/senhas/except') == False:
                 os.system('touch /etc/setup/senhas/except')
                 os.system('''echo 'Ola anqui fica armazenado todas as senhas'''\
                          +'''criadas por este progama.' > /etc/setup/senhas/except''')
                  
              if os.path.isfile('/etc/setup/limite/lm') == False:
                 os.system('touch /etc/setup/limite/lm')
              if t == True:
                 os.system('apt-get autoremove ssh -y')
                 os.system('apt-get update && apt-get install ssh -y')
              if s == True:
                 os.system('apt-get autoremove squid3 -y')

              os.system('apt-get install squid3 -y')
              sleep(2)
              if os.path.isfile('/etc/squid/squid.conf.save') == True\
               or os.path.isfile('/etc/squid3/squid.conf.save') == True:
               	 if os.path.isfile('/etc/squid/squid.conf.save') == True:
               	    os.system('rm -rf /etc/squid3/squid.conf.save')
               	    os.system('touch /etc/squid3/squid.conf')
               	 else:
               	    os.system('rm -rf /etc/squid/squid.conf.save')
              ...
              if os.path.isfile('/etc/squid/squid.conf') == True:
                 ...
                 if os.path.isfile('/etc/squid/domains') == False:
                    os.system('touch /etc/squid/domains')
                    os.system('''echo '.vivo.com.br\n.claro.com.br\n.oi.com.br\
                              \n.tim.com.br' >> /etc/squid/domains/domain''')
                    os.system('touch /etc/squid/domains/.claro.com.br')
                    os.system('touch /etc/squid/domains/.vivo.com.br')
                    os.system('touch /etc/squid/domains/.tim.com.br')
                    os.system('touch /etc/squid/domains/.oi.com.br')
                    os.system('rm -rf /etc/squid/squid.conf')
                    os.system('touch /etc/squid/squid.conf')
                    os.system('''echo '# ACL DE CONEXÃO\n\nacl accept src '''+__ip__\
                             +'''\nacl ip url_regex -i '''+__ip__\
                             +'''\nacl payload dstdomain -i "/etc/squid3/domains/domain"\n'\
                              > /etc/squid3/squid.conf''')
                    os.system('''echo '# PORTAS DE ACESSO\n\nhttp_port 80\nhttp_port 8080\
                              \nhttp_port 8799\nhttp_port 3128\n\n# Nome do servidor\
                 ...
                 if os.path.isfile('/etc/squid3/domains') == False:
                    os.system('touch /etc/squid3/domains')
                    os.system('''echo '.vivo.com.br\n.claro.com.br\n.oi.com.br\
                             \n.tim.com.br' >> /etc/squid3/domains/domain''')
                    os.system('touch /etc/squid3/domains/.claro.com.br')
                    os.system('touch /etc/squid3/domains/.vivo.com.br')
                    os.system('touch /etc/squid3/domains/.tim.com.br')
                    os.system('touch /etc/squid3/domains/.oi.com.br')
                    os.system('rm -rf /etc/squid3/squid.conf')
                    os.system('touch /etc/squid3/squid.conf')
                    os.system('''echo '# ACL DE CONEXÃO\n\nacl accept src '''+__ip__\
                             +'''\nacl ip url_regex -i '''+__ip__\
                             +'''\nacl payload dstdomain -i "/etc/squid3/domains/domain"\n'\
           ...
           if w == '3':
              if os.path.isfile('/etc/squid3/domains/domain') == False\
               or os.path.isfile('/etc/squid/domains/domain') == False:
                 if os.path.isfile('/etc/squid/squid.conf') == True:
                    os.system('touch /etc/squid/domains/domain')
                 else:
                    os.system('touch /etc/squid3/domains/domain')
              print (f2+'Domains do arquivo:\033[36m')
              if os.path.isfile('/etc/squid/domains/domain') == True:
                 os.system('cat /etc/squid/domains/domain')
              else:
                 os.system('cat /etc/squid3/domains/domain')
              def dom():
                  ...
                  if os.path.isfile('/etc/squid3/domains/domain') == True:
                     if os.path.isfile('/etc/squid3/domains/%s' %(do)) == True:
                        ...
                        dom()
                     os.system('''echo '%s' >> /etc/squid3/domains/domain''' % (do))
                     os.system('''echo '%s' >> /etc/squid3/domains''' % (do))
                     os.system('cd /etc && service squid3 restart')
                     ...
                     if os.path.isfile('/etc/squid/domains/%s' %(do)) == True:
                        print (f1+'Erro: o domain %s ja existe.\033[m' %(do))
                        dom()
                     os.system('''echo '%s' >> /etc/squid/domains/domain''' % (do))
                     os.system('''echo '%s' >> /etc/squid/domains''' % (do))
                     os.system('cd /etc && service squid restart')
                     print (f2+'Domain: %s Adicionado.. [0] Para Home.' % (do))
                     dom()
              dom()
              ...
              def delet():
                  ...
                  if os.path.isfile('/etc/squid/domains/domain') == True:
                     if os.path.isfile('/etc/squid/domains/%s' %(de)) == False:
                        ...
                        delet()
                     os.system('rm -rf /etc/squid/domains/%s' %(de))
                     ...
                     if os.path.isfile('/etc/squid3/domains/%s' %(de)) == False:
                        print (f1+'Erro: o domain %s nao existe.\033[m' %(de))
                        delet()
                     os.system('rm -rf /etc/squid3/domains/%s' %(de))
           ...
           if w == '7':
              ...
              def user():
                  ...
                  os.system("d=$(date '+%C%y-%m-%d' -d '+"+dx+" days')\
                  ...
                  os.system('''echo '%s - maxlogins %s' >> /etc/setup/limite/%s''' %(n,lm,n))
                  os.system('''echo '%s - maxlogins %s' >> /etc/security/limits.conf'''\
                   %(n,lm))
           ...
           if w == '9':
              def dex():
                  ...
                  if os.path.isfile('/etc/setup/senhas/%s' %(df)) == False:
                     ...
                     dex()
                  os.system('userdel --force %s > /dev/null 2>/dev/null' %(df))
                  os.system('rm -rf /etc/setup/senhas/%s' %(df))
                  ...
                  def delete():
                      ...
                      f.close()
                  delete()
                  os.system('rm -rf /etc/setup/limite/'+df)
           ...
           if w == '8':
              def rdf():
                   ...
                   if md == '1':
                      ...
                      os.system('(echo "'+sna+'" ; echo "'+sna\
                               +'" ) |passwd '+rd+' > /dev/null 2>/dev/null')
                      os.system('rm -rf /etc/setup/senhas/'+rd)
                      os.system('echo "'+sna+'" > /etc/setup/senhas/'+rd)
                   ...
                   if md == '2':
                      ...
                      os.system('chage -E '+dt+' '+rd+' 2> /dev/null')
                   ...
                   if md == '3':
                      ...
                      def delete():
                          ...
                          f.close()
                      delete()
                      os.system('rm -rf /etc/setup/limite/'+rd)
                      os.system('''echo '%s - maxlogins %s' >> /etc/setup/limite/%s'''\
                       %(rd,mt,rd))
                      os.system('''echo '%s - maxlogins %s' >> /etc/security/limits.conf'''\
                       %(rd,mt))
